...
def get_rec(my_profile, num_of_rec):
    ...
    # set rating_x in each row to 1
    df['rating_x'] = 1

    # rename columns
    df = df.rename(columns={'rating_x': 'interaction', 'name': 'location_name'})
    ...
    interactions_full_df = df.groupby(['user_id', 'location_id'])['interaction'].sum().apply(
    ...
    def most_popular_items(interactions_df, item_id, topn=10):
        # Get a count of user_ids for each unique location as
        # a series of location ids and popularity
        item_popularity_df = interactions_df.groupby(item_id)['interaction'].sum().sort_values(
    ...
    try:
        class PopularityRecommender:
            ...
            def recommend_items(self, user_id, items_to_ignore=[], topn=10, verbose=False):
                recommendations_df = self.popularity_df[~self.popularity_df['location_id'].isin(items_to_ignore)] \
        ...
        popularity_model = PopularityRecommender(most_popular_items(interactions_full_df, 'location_id'))
    ...
    try:
        ...
        item_ids = df['location_id'].tolist()
        tfidf_matrix = vectorizer.fit_transform(df['location_name'] + "" + df['description'] + "")
        ...
        def build_users_profile(person_id, interactions_indexed_df):
            ...
            user_item_profiles = get_item_profiles(interactions_person_df['location_id'])

            user_item_strengths = np.array(interactions_person_df['interaction']).reshape(-1, 1)
        ...
        def build_users_profiles():
            interactions_indexed_df = new_train_df[
                new_train_df['location_id'].isin(df['location_id'])].set_index('user_id')
            user_profiles = {}
        ...
        class ContentBasedRecommender:
            ...
            def recommend_items(self, user_id, items_to_ignore=[], topn=10, verbose=False):
                ...
                recommendations_df = pd.DataFrame(similar_items_filtered, columns=['location_id', 'recStrength'])
                # remove duplicates
                recommendations_df = recommendations_df.drop_duplicates(subset=['location_id'])
                # sort by recStrength
                ...
                if verbose:
                    ...
                    recommendations_df = recommendations_df.merge(self.items_df, how='left',
                                                                  left_on='location_id',
                                                                  right_on='location_id')[
                        ['recStrength', 'location_id', 'location_name']]

                return recommendations_df.drop_duplicates(subset=['location_id'])
        ...
        class HybridRecommender:
            ...
            def recommend_items(self, user_id, items_to_ignore=[], topn=10, verbose=False):
                ...
                recs_df = cb_recs_df.merge(cf_recs_df,
                                           how='outer',
                                           left_on='location_id',
                                           right_on='location_id').fillna(0.0)
                ...
                # Recommend the highest predicted rating movies that the user hasn't seen yet.
                recommendations_df = recommendations_df[~recommendations_df['location_id'].isin(items_to_ignore)] \
                ...
                if verbose:
                    ...
                    recommendations_df = recommendations_df.merge(self.items_df, how='left',
                                                                  left_on='location_id',
                                                                  right_on='location_id')[
                        ['recStrengthHybrid', 'location_id', 'location_name']]

                return recommendations_df.drop_duplicates(subset=['location_id']).reset_index(drop=True)
    ...
    if my_profile in user_profiles:
        ...
        return recommadation.head(num_of_rec)['location_id'].tolist(), 1

    else:
        # return top 10 popular items
        item_popularity_df = popularity_model.recommend_items(my_profile, topn=10, verbose=False)
        item_popularity_df_head = item_popularity_df.head(num_of_rec)['location_id'].tolist()
...
# print(get_rec("652b9e279c8deef2485bf8f5", 3))
